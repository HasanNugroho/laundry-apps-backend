...
class DashboardController extends Controller
{
    ...
    {
        $user_outlet = Auth::user()->outlet_id;
        $keuangan = DB::select('select ps.*, o.jenis, o.jenis_service, o.kasir, o.keterangan, o.item_name as namaBarang, o.satuan, o.harga as hargaBarang, o.jumlah as jumlahBarang,  o.nominal, o.outletid, os.nama_outlet, se.nama_layanan ,se.harga, se.jenis, se.item, o.created_at as operasionalCreatedDate, o.updated_at as opeasionalUpdatedDate, pb.status as statusPembayaran from operasionals o left JOIN outlets os on o.outletid = os.id left JOIN pesanans ps on o.idpesanan = ps.id LEFT JOIN services se on ps.idlayanan = se.id left join pembayarans pb on ps.id = pb.idpesanan where (ps.status != \'DIBATALKAN\' and (pb.status != \'BELUM BAYAR\' and pb.status != \'UTANG\') or o.jenis = \'PENGELUARAN\') and o.outletid = \'' . $user_outlet . '\' and date(o.created_at) = \'' . date('Y-m-d') . '\'');
        
        $totalPemasukan = DB::select('select sum(o.nominal) as totalPemasukan from operasionals o left JOIN outlets os on o.outletid = os.id left JOIN pesanans ps on o.idpesanan = ps.id LEFT JOIN services se on ps.idlayanan = se.id left join pembayarans pb on ps.id = pb.idpesanan where o.outletid = \'' . $user_outlet . '\' and date(o.created_at) = \'' . date('Y-m-d') . '\' and o.jenis = \'PEMASUKAN\' and (pb.status != \'BELUM BAYAR\' and pb.status != \'UTANG\')');
